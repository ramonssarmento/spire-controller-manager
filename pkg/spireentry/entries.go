...
func renderStaticEntry(spec *spirev1alpha1.ClusterStaticEntrySpec) (*spireapi.Entry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse SPIFFEID: %v", err)
	}
	parentID, err := spiffeid.FromString(spec.ParentID)
	if err != nil {
		return nil, fmt.Errorf("failed to parse ParentID: %v", err)
	}
	selectors, err := parseSelectors(spec.Selectors)
	if err != nil {
		return nil, fmt.Errorf("failed to parse Selectors: %v", err)
	}
