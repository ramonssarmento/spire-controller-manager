...
func (r *entryReconciler) reconcile(ctx context.Context) {
	...
	if err := r.addClusterStaticEntryEntriesState(ctx, state, clusterStaticEntries); err != nil {
		log.Error(err, "Failed to add entry state for ClusterSPIFFEIDs")
		return
	}
	...
	if err := r.addClusterSPIFFEIDEntriesState(ctx, state, clusterSPIFFEIDs); err != nil {
		log.Error(err, "Failed to add entry state for ClusterSPIFFEIDs")
		return
	}
...
func (r *entryReconciler) addClusterStaticEntryEntriesState(ctx context.Context, state entriesState, clusterStaticEntries []*ClusterStaticEntry) error {
	log := log.FromContext(ctx)
	for _, clusterStaticEntry := range clusterStaticEntries {
...
func (r *entryReconciler) addClusterSPIFFEIDEntriesState(ctx context.Context, state entriesState, clusterSPIFFEIDs []*ClusterSPIFFEID) error {
	log := log.FromContext(ctx)
	for _, clusterSPIFFEID := range clusterSPIFFEIDs {
		log := log.WithValues(clusterSPIFFEIDLogKey, objectName(clusterSPIFFEID))
...
func sortDeclaredEntriesByPreference(entries []declaredEntry) {
	...
	sort.Slice(entries, func(i, j int) bool {
		...
		aKind := a.GetObjectKind()
		bKind := b.GetObjectKind()

		fmt.Println(aKind, bKind)
		if aKind != bKind {
		}
